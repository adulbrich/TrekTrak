<script lang="ts">
  ...
  function filterParticipants() {
    ...
    rows.forEach((row) => {
      ...
      const teamMembers = Array.from(row.querySelectorAll(".hidden li")).map((member) => member.innerText.toLowerCase());
      const display = teamName.includes(searchQuery.toLowerCase()) || teamMembers.some((memberName) => memberName.includes(searchQuery.toLowerCase())) ? "table-row" : "none";
...
<svelte:head>
  ...
  <title>Event | Top Teams & Participants</title>
...
<div class="container ml-[16%] w-auto">
  ...
  <div class="max-w-full p-4 mx-auto">
    <!-- Search Bar -->
    <input type="text" placeholder="Search participants..." class="w-full p-2 border border-gray-300 rounded-md mb-4" bind:value={searchQuery} on:input={() => filterParticipants()} />
